<template>
  <q-card v-if="exception !== null">
    <q-card-actions v-if="props.exceptions.length > 1">
      <q-btn flat
             v-if="indexToView !== null && indexToView !== undefined && indexToView < (props.exceptions.length - 1)"
             @click="indexToView++">Previous exception
      </q-btn>
      <q-space></q-space>
      <q-btn flat v-if="indexToView !== null && indexToView !== undefined && indexToView !== 0"
    ...
    <q-card-section>
      ...
      <div class="text-subtitle2">{{ exception.file }}:{{ exception.line }}</div></q-card-section>
