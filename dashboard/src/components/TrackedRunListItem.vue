  ...
  <q-item clickable :to="{name: 'run.show', params: {jobStatusId: props.trackedRun.id}}">
    ...
    <q-item-section top>
      ...
      <q-item-label caption>
        <span v-if="props.trackedRun.messages.length > 0">
          {{props.trackedRun.messages.length }} {{ props.trackedRun.messages.length > 1 ? 'messages' : 'message'}}.
        </span>
        <span v-if="props.trackedRun.signals.length > 0">
          {{props.trackedRun.signals.length }} {{ props.trackedRun.signals.length > 1 ? 'signals' : 'signal'}}.
        </span>
      </q-item-label>
    </q-item-section>
